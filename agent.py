...
def _analytics_defaults():
    return {
        ...
        # Detailed events are recorded from this version onward.
        # Older total counters remain untouched.
        "event_history": [],
...
def load_analytics():
    ...
    try:
        ...
        if not isinstance(saved.get("traffic_sources"), dict):
            saved["traffic_sources"] = defaults_data["traffic_sources"].copy()
        for source, count in defaults_data["traffic_sources"].items():
            saved["traffic_sources"].setdefault(source, count)

        if not isinstance(saved.get("event_history"), list):
            saved["event_history"] = []
...
def _local_now():
    return datetime.now(timezone.utc).astimezone(ANALYTICS_TZ)


def _parse_event_local_time(entry):
    try:
...
def track_event(event_name, amount=1, traffic_source=None, detail=None):
    # Admin Test Mode protects analytics while the owner is testing.
    ...
    if event_name in data and isinstance(data[event_name], int):
        data[event_name] += amount

    source = normalize_traffic_source(
        traffic_source if traffic_source is not None else current_traffic_source()
    )

    # Traffic source totals count sessions/app opens only. This avoids
    # making one active visitor look like many visitors because of clicks.
    if traffic_source is not None:
        data["traffic_sources"][source] = (
            data["traffic_sources"].get(source, 0) + amount
        )

    history = data.setdefault("event_history", [])
    ...
    # Keep enough history for useful trends without growing forever.
    data["event_history"] = history[-3000:]
    save_analytics(data)
...
def filtered_event_history(usage, period="Today", source_filter="All", event_filter="All"):
    history = usage.get("event_history", [])
    ...
    for entry in reversed(history):
        local_dt = _parse_event_local_time(entry)
        if local_dt is None:
            continue

        if period == "Today" and local_dt.date() != today:
            continue
        if period == "Yesterday" and local_dt.date() != yesterday:
            continue
        if period == "Last 7 Days" and local_dt < now_local - timedelta(days=7):
            continue

        source = entry.get("source", "Direct")
        label = entry.get(
            "event_label",
            EVENT_LABELS.get(entry.get("event"), str(entry.get("event", "Event"))),
        )

        if source_filter != "All" and source != source_filter:
            continue
        if event_filter != "All" and label != event_filter:
            continue
